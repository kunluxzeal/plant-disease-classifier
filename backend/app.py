...
# Import your existing modules - WITHOUT AUDIO
from utils import analyze_image_with_openai_chat
# REMOVED: from AudioToS3 import UploadToS3
        ...
        with self._lock:
            ...
            try:
                ...
                if plant_model_path.exists():
                    ...
                    self.models_available = False
                    print(f"⚠️ Model files not found at: {plant_model_path}")
                    print("Running in online-only mode.")
...
def predict_offline(image_data: bytes):
    ...
    try:
        ...
        result = {
            ...
            "disease_confidence": 0.0,
            "is_healthy": False,
            "message": "Offline mode - basic detection only",
            "treatment": None
        }
...
async def predict_online(image_data: bytes):
    """Run online prediction asynchronously (no audio features)"""
    try:
        # Create temporary file
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            tmp.write(image_data)
        ...
        try:
            ...
            # Run OpenAI analysis in thread pool
            loop = asyncio.get_event_loop()
            treatment_json = await loop.run_in_executor(
                executor,
                analyze_image_with_openai_chat,
            ...
            if not isinstance(treatment_json, dict):
                treatment_json = {
                    ...
                    "raw_text": str(treatment_json)
                }

            return {
                "mode": "online",
                "analysis": treatment_json,
                "models_available": model_loader.models_available
            }
...
def health_check():
    return {
        ...
        "models_available": model_loader.models_available,
        "openai_configured": bool(OPENAI_API_KEY),
        "available_modes": ["mock"] if not model_loader.models_available and not OPENAI_API_KEY else ["online"] if OPENAI_API_KEY else ["offline"],
        "tensorflow_version": tf.__version__,
        "active_threads": threading.active_count()
    }
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        has_models = model_loader.models_available
        
        # Determine mode
        if has_openai and internet_connected():
            mode = "online"
        elif has_models:
            mode = "offline"
        else:
        ...
        if mode == "offline":
            if not has_models:
                raise HTTPException(
                    status_code=503, 
                    detail="Offline mode not available - models not loaded"
                )
            ...
            return {
                ...
                "is_healthy": False,
                "message": "Mock response - configure OpenAI API for real analysis"
            }
